...
class CorrectStudentAdmissionNumbers extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'students:correct-admission-numbers {--school= : Specific school ID to process} {--dry-run : Report matches without modifying database}';
     ...
     * @var string
     */
    protected $description = 'Audit and unify duplicate student records across academic sessions to enforce permanent immutable admission numbers.';
    ...
    public function handle()
    {
        ...
        if (!empty($schoolOption)) {
            $schoolsQuery->where('id', (int) $schoolOption);
        }
        ...
        foreach ($schools as $school) {
            ...
            if ($students->isEmpty()) {
                $this->line("  No student records found.");
                continue;
            }

            // Group by Full Name + Father/Guardian Name + DOB
            $groupedByIdentity = [];
            ...
            foreach ($groupedByIdentity as $identityKey => $stGroup) {
                ...
                $masterAdmissionNumber = $primaryStudent->admission_number;
                ...
                $primaryName = trim($primaryStudent->first_name . ' ' . $primaryStudent->last_name);
                $this->line("  [MATCH] Student: <comment>{$primaryName}</comment> (DOB: {$primaryStudent->date_of_birth}, Father: {$primaryStudent->father_name})");
                $this->line("    Master Record ID: {$primaryStudent->id} | Master Admission No: <info>{$masterAdmissionNumber}</info>");

                foreach ($secondaryStudents as $secSt) {
                    $secSessionName = $sessionNameMap[$secSt->academic_session_id] ?? "Session #{$secSt->academic_session_id}";
                    $this->line("    Duplicate Record ID: {$secSt->id} | Session: {$secSessionName} | Old Adm No: <error>{$secSt->admission_number}</error> -> Unifying into <info>{$masterAdmissionNumber}</info>");
                }
                ...
                $schoolUnified++;
                $schoolRemoved += count($secondaryStudents);
            }
            ...
            $this->info("  School Results: {$schoolUnified} student identities unified, {$schoolRemoved} duplicate records removed.");
            $totalUnifiedCount += $schoolUnified;
            $totalRemovedCount += $schoolRemoved;
        }

        $this->newLine();
        if ($dryRun) {
            $this->warn("DRY RUN COMPLETE: {$totalUnifiedCount} student identities would be unified, {$totalRemovedCount} duplicate records would be merged.");
        } else {
            $this->info("SUCCESS: {$totalUnifiedCount} student identities successfully unified across academic sessions. {$totalRemovedCount} duplicate records merged.");
        }

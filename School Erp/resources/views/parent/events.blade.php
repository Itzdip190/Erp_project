...
<head>
    ...
    <title>School Calendar & Events — SchoolCloud ERP</title>
...
.event-banner.holiday{background:var(--red);}
.event-banner.event{background:var(--blue);}
.event-body{
    padding:18px;
    flex:1;
}
...
.event-type-badge.holiday{background:rgba(239,68,68,0.1);color:var(--red);}
.event-type-badge.event{background:rgba(59,130,246,0.1);color:var(--blue);}
...
<div class="main">
    ...
    <nav class="topbar">
        <div class="topbar-left">
            ...
            <div class="greeting">
                ...
                <p>Track academic sessions, scheduled events, holiday closures, and exhibition dates</p>
            </div>
        </div>
    ...
    <div class="pg">
        <div class="card">
            <div class="card-hdr">
                <span class="card-title"><i class="fas fa-calendar" style="color:var(--gold);margin-right:8px;"></i>Upcoming Events & Holiday Schedules</span>
            </div>
            <div class="card-body" style="background:#fff;">
                <div class="event-grid">
                    @forelse($events as $event)
                    <div class="event-card">
                        <div class="event-banner {{ $event->is_holiday ? 'holiday' : 'event' }}"></div>
                        <div class="event-body">
                            <span class="event-type-badge {{ $event->is_holiday ? 'holiday' : 'event' }}">
                                <i class="fas {{ $event->is_holiday ? 'fa-umbrella-beach' : 'fa-calendar-star' }}" style="margin-right:4px;"></i>
                            ...
                            <p class="event-desc">{{ $event->description ?? 'No details provided.' }}</p>
                        </div>
                    </div>
                    @empty
                    <div style="text-align:center;padding:60px 20px;color:var(--t3);grid-column: 1 / -1;">
                        <i class="fas fa-calendar-xmark" style="font-size:42px;display:block;margin-bottom:12px;color:var(--border);"></i>
                        <span style="font-size:13px;">No upcoming school events or holiday notices are currently registered.</span>
                    </div>
                    @endforelse
                </div>
            </div>

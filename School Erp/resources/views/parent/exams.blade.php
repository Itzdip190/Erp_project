...
<head>
    ...
    <title>Exams & Report Cards — SchoolCloud ERP</title>
...
<div class="main">
    ...
    @include('parent.partials.topbar', [
        'title' => 'Exams & Grades',
        'subtitle' => 'Academic performance reports for student ' . $student->full_name
    ])
    ...
    <div class="pg">
        ...
        @php
            ...
            $maxTotal = $marks->sum('max_marks');
            $avgScore = $maxTotal > 0 ? round(($totalMarks / $maxTotal) * 100, 1) : 0;
            $gpa = $avgScore >= 90 ? 'A+' : ($avgScore >= 80 ? 'A' : ($avgScore >= 70 ? 'B' : ($avgScore >= 60 ? 'C' : ($avgScore >= 40 ? 'D' : 'F'))));
        @endphp
        <div class="stats-row">
            <div class="stat">
                <div class="stat-ico" style="background:rgba(59,130,246,0.1); color:var(--blue);"><i class="fas fa-chart-bar"></i></div>
                <div>
            ...
            <div class="stat">
                <div class="stat-ico" style="background:rgba(16,185,129,0.1); color:var(--green);"><i class="fas fa-award"></i></div>
                <div>
            ...
            <div class="stat">
                <div class="stat-ico" style="background:rgba(245,158,11,0.1); color:var(--gold);"><i class="fas fa-graduation-cap"></i></div>
                <div>
                    <div class="stat-val">{{ $marks->count() }}</div>
                    <div class="stat-lbl">Tests Evaluated</div>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-hdr" style="display:flex; justify-content:space-between; align-items:center;">
                ...
                <button onclick="window.print()" class="btn btn-outline" style="padding:6px 12px; font-size:11.5px;"><i class="fas fa-print"></i> Print Grade Sheet</button>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            @empty
                            <tr>
                                <td colspan="7" style="text-align:center; padding:40px; color:var(--t3);">No assessment marks entered yet.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>

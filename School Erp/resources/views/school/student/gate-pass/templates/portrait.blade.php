{{-- PORTRAIT COMPACT SLIP GATE PASS TEMPLATE --}}
@php
...
<div class="gate-pass-sheet portrait-theme" style="background:#ffffff; color:#1e293b; font-family:'Segoe UI', Arial, sans-serif; border-radius:8px; box-shadow:0 6px 20px rgba(0,0,0,0.06); width:100%; max-width:440px; margin:0 auto; box-sizing:border-box; border:1px solid #cbd5e1; padding:20px;">
    ...
    <div style="display:flex; justify-content:space-between; font-size:11px; margin-bottom:10px; color:#475569; font-weight:600;">
        <div>Pass: <strong id="pv-ref-no-portrait" style="color:#0f172a;">{{ $autoNumber ?? '2026/GP/1' }}</strong></div>
    ...
    <div style="display:flex; gap:12px; margin-bottom:12px; align-items:center; background:#f8fafc; padding:8px 10px; border-radius:6px; border:1px solid #e2e8f0;">
        <div style="width:50px; height:60px; border-radius:4px; overflow:hidden; border:1px solid #cbd5e1; flex-shrink:0;">
            <img id="pv-student-photo-portrait" src="{{ $student->photo_url }}" alt="Photo" style="width:100%; height:100%; object-fit:cover; {{ !$student->photo ? 'display:none;' : '' }}">
            <div id="pv-student-photo-placeholder-portrait" style="display:{{ $student->photo ? 'none' : 'flex' }}; height:100%; align-items:center; justify-content:center; color:#94a3b8; font-size:9px;">
                Photo
            </div>
        </div>
        <div style="font-size:11.5px; line-height:1.4;">
            <div><strong id="pv-student-name-portrait" style="color:#0f172a; font-size:13px;">{{ $student->full_name }}</strong></div>
            <div style="color:#64748b;">Adm: <strong id="pv-adm-no-portrait" style="color:#334155;">{{ $student->admission_number }}</strong> &bull; Class: <strong id="pv-class-sec-portrait" style="color:#334155;">{{ $student->class?->name ?? '' }} {{ $student->section?->name ?? '' }}</strong></div>
        </div>
    </div>

    <div style="font-size:11.5px; margin-bottom:14px; border:1px solid #e2e8f0; border-radius:6px; padding:8px 10px;">
        <div style="margin-bottom:4px;"><span style="color:#64748b;">Guardian:</span> <strong id="pv-guardian-name-portrait" style="color:#0f172a;">{{ $student->father_name ?? 'Father' }}</strong> (<span id="pv-guardian-relation-portrait">Father</span>)</div>
        <div><span style="color:#64748b;">Reason:</span> <strong id="pv-reason-portrait" style="color:#2563eb;">Medical Checkup / Illness</strong></div>
    </div>

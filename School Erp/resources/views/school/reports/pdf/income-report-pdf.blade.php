...
<head>
    <meta charset="utf-8">
    <title>{{ $reportTitle ?? 'Non-Fee Income Ledger & Audit Report' }}</title>
    <style>
        @page {
...
<body>
    ...
    <table class="header-table">
        <tr>
            ...
            <td class="text-right" style="vertical-align: middle;">
                <div style="font-size: 10px; font-weight: bold; color: #059669;">OFFICIAL INCOME REPORT</div>
    ...
    <table class="data-table">
        <thead>
            <tr>
                ...
                <th style="width: 18%;">Income Head</th>
        ...
        <tbody>
            ...
            @forelse($incomes as $idx => $inc)
                @php
                    $amt = (float) $inc->amount;
                    if ($inc->status !== 'cancelled') $totalAmt += $amt;
                @endphp
                <tr>
                    <td>{{ $idx + 1 }}</td>
                    <td style="font-weight: bold;">{{ $inc->receipt_no ?? ($inc->reference_no ?? ('INC-' . $inc->id)) }}</td>
                    <td>{{ $inc->income_date ? \Carbon\Carbon::parse($inc->income_date)->format('d M Y') : '—' }}</td>
                    <td style="font-weight: bold; color: #065f46;">{{ optional($inc->incomeHead)->name ?? 'Other' }}</td>
                    <td>{{ $inc->category_label }}</td>
                    <td>{{ ucfirst(str_replace('_', ' ', $inc->payment_mode ?? 'Cash')) }}</td>
                    <td class="text-right" style="font-weight: bold;">₹{{ number_format($amt, 2) }}</td>
                    <td>{{ $inc->received_from ?? '—' }}</td>
                    <td class="text-center">
                        <span class="badge {{ $inc->status === 'paid' ? 'badge-success' : ($inc->status === 'pending' ? 'badge-warning' : 'badge-danger') }}">
                            {{ ucfirst($inc->status ?? 'Paid') }}
                        </span>
                    </td>
                </tr>
            @empty
            ...
            @if(count($incomes) > 0)
                <tr class="total-row">
                    <td colspan="6" class="text-right">GRAND TOTAL INCOME:</td>
    ...
    <div class="footer-note">
        {{ $school->name ?? 'School ERP' }} — Non-Fee Income Ledger & Audit Report — Confidentially Generated Record
    </div>

...
<div class="page-hdr">
    <div class="page-hdr-left">
        <h1><i class="fas fa-users" style="color:var(--gold);margin-right:8px;"></i>Parent Download Status</h1>
...
<div class="card" style="margin-bottom:20px;">
    <div class="card-body" style="padding:15px 20px;">
        <form method="GET" action="{{ route('school.downloads.parent-status') }}" style="display:flex; justify-content:space-between; align-items:flex-end; gap:15px; flex-wrap:wrap;">
            ...
            <div style="display:flex; gap:8px;">
                <button type="button" class="btn btn-outline" style="border-color:#b45309; color:#b45309; padding:8px 16px; font-size:13px;" onclick="showToast('Loading Parent Download Status...')">
                    <i class="fas fa-eye"></i> VIEW
                </button>
                <button type="button" class="btn btn-outline" style="border-color:#b45309; color:#b45309; padding:8px 16px; font-size:13px;" onclick="showToast('Downloading Parent Download Status...')">
        ...
        <div class="card" style="margin-bottom:0; background:{{ $tab === 'logged_in' ? 'rgba(16,185,129,0.1)' : 'rgba(16,185,129,0.03)' }}; border:1px solid {{ $tab === 'logged_in' ? '#10b981' : 'transparent' }}; border-left:5px solid #10b981; cursor:pointer; transition:transform 0.2s;">
            <div class="card-body" style="padding:18px; display:flex; align-items:center; gap:15px;">
                ...
                <div>
                    ...
                    <span style="font-size:12px; color:#047857; font-weight:700;">Parents who have logged in</span>
                </div>
            </div>
        </div>
    </a>
        ...
        <div class="card" style="margin-bottom:0; background:{{ $tab === 'not_logged_in' ? 'rgba(239,68,68,0.1)' : 'rgba(239,68,68,0.03)' }}; border:1px solid {{ $tab === 'not_logged_in' ? '#ef4444' : 'transparent' }}; border-left:5px solid #ef4444; cursor:pointer; transition:transform 0.2s;">
            <div class="card-body" style="padding:18px; display:flex; align-items:center; gap:15px;">
                ...
                <div>
                    ...
                    <span style="font-size:12px; color:#b91c1c; font-weight:700;">Parents who haven't logged in</span>
                </div>
            </div>
        </div>
    </a>
            ...
            <tbody>
                @forelse($activeList as $index => $st)
                    <tr>
                        ...
                        <td>
                            <span class="badge badge-blue">{{ $st->admission_number }}</span>
                        </td>
                        <td>
                ...
                @empty
                    <tr>
                        <td colspan="6" style="text-align:center; padding:40px; color:var(--t3);">
                            <i class="fas fa-user-friends" style="font-size:32px; color:var(--t3); margin-bottom:12px; display:block;"></i>
                            No parent records match the criteria.
                        </td>
                    </tr>
                @endforelse
            </tbody>

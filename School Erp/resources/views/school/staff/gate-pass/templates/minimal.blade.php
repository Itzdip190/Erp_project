{{-- MINIMAL STAFF GATE PASS TEMPLATE --}}
@php
...
<div class="gate-pass-sheet minimal-theme" style="background:#ffffff; color:#1e293b; font-family:'Inter', -apple-system, BlinkMacSystemFont, sans-serif; border-radius:6px; box-shadow:0 4px 15px rgba(0,0,0,0.05); width:100%; max-width:720px; margin:0 auto; box-sizing:border-box; border:1.5px solid #334155; padding:24px 28px;">
        ...
        <div style="display:flex; align-items:center; gap:12px;">
            ...
            <div>
                ...
                <div style="font-size:11px; color:#64748b;">
                    <span id="pv-school-address-minimal">{{ $schoolAddress ?? $staff->school?->address ?? '' }}</span>
                    <span id="pv-school-city-state-minimal">{{ ($schoolCity ?? '') ? ', ' . $schoolCity : '' }}</span>
                </div>
            </div>
        </div>
        <div style="text-align:right;">
            <div style="font-size:14px; font-weight:900; letter-spacing:1px; text-transform:uppercase; color:#0f172a;">STAFF OUT PASS</div>
            <div style="font-size:11px; font-family:monospace; color:#475569; font-weight:700;">#<span id="pv-ref-no-minimal">{{ $autoNumber ?? '2026/SGP/1' }}</span></div>
        </div>
    </div>
    ...
    <div style="display:flex; justify-content:space-between; font-size:12px; margin-bottom:16px; color:#475569; border-bottom:1px solid #e2e8f0; padding-bottom:8px;">
        ...
        <div>Status: <strong style="color:#0f172a; text-transform:uppercase;">AUTHORIZED EXIT</strong></div>
    </div>
    ...
    <div style="display:flex; gap:16px; margin-bottom:18px; align-items:flex-start;">
        <div style="flex-grow:1;">
            <table style="width:100%; border-collapse:collapse; font-size:12.5px;">
                <tr style="border-bottom:1px solid #f1f5f9; height:26px;">
                    <td style="width:140px; color:#64748b; font-weight:600;">Staff Name</td>
                    <td id="pv-staff-name-minimal" style="font-weight:700; color:#0f172a;">{{ $staff->full_name }}</td>
                </tr>
                <tr style="border-bottom:1px solid #f1f5f9; height:26px;">
                    <td style="color:#64748b; font-weight:600;">Employee ID</td>
                    <td id="pv-emp-id-minimal" style="color:#0f172a; font-weight:700;">{{ $staff->employee_id }}</td>
                </tr>
                <tr style="border-bottom:1px solid #f1f5f9; height:26px;">
                    <td style="color:#64748b; font-weight:600;">Designation / Dept</td>
                    <td id="pv-desig-dept-minimal" style="color:#0f172a;">{{ optional($staff->designation)->name ?? 'Staff' }} ({{ optional($staff->department)->name ?? 'General' }})</td>
                </tr>
                <tr style="border-bottom:1px solid #f1f5f9; height:26px;">
                    <td style="color:#64748b; font-weight:600;">Reason</td>
                    <td id="pv-reason-minimal" style="font-weight:700; color:#0f172a;">Official School Duty</td>
                </tr>
                <tr style="height:26px;">
                    <td style="color:#64748b; font-weight:600;">Expected Return</td>
                    <td id="pv-return-time-minimal" style="color:#0f172a; font-weight:600;">--</td>
                </tr>
            </table>
        </div>
        ...
        <div style="flex-shrink:0; width:80px; height:96px; border:1px solid #cbd5e1; border-radius:4px; overflow:hidden; background:#f8fafc; text-align:center;">
            <img id="pv-staff-photo-minimal" src="{{ $staff->photo_url }}" alt="Photo" style="width:100%; height:100%; object-fit:cover; {{ !$staff->photo ? 'display:none;' : '' }}">
            <div id="pv-staff-photo-placeholder-minimal" style="display:{{ $staff->photo ? 'none' : 'flex' }}; height:100%; align-items:center; justify-content:center; color:#94a3b8; font-size:10px;">
                Photo
            </div>
        </div>
    </div>
    ...
    <!-- Signatures Row -->
    <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap:16px; text-align:center; padding-top:20px; border-top:1px solid #0f172a; font-size:11px;">
        <div>

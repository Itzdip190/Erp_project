...
<body>
    <div class="sidebar">
        ...
        <a href="{{ route('teacher.assignments.index') }}" style="color:#f59e0b;text-decoration:none;padding:10px;display:block;font-weight:700;background:rgba(255,255,255,.08);border-radius:8px;"><i class="fas fa-tasks me-2"></i> Class Assignments</a>
        <a href="{{ route('teacher.study-materials.index') }}" style="color:rgba(255,255,255,.8);text-decoration:none;padding:10px;display:block;font-weight:600;margin-top:4px;"><i class="fas fa-book-open me-2"></i> Study Materials</a>
    </div>
    <div class="main">
        <div class="hdr">
            <div>
                <h2 style="font-family:'Plus Jakarta Sans';font-size:22px;font-weight:800;">Class Assignments</h2>
                <p style="color:#6b7280;font-size:13px;">Create and assign homework tasks for your classes</p>
            </div>
            <a href="{{ route('teacher.assignments.create') }}" class="btn btn-blue"><i class="fas fa-plus"></i> Create New Assignment</a>
        </div>
        ...
        <div class="card">
                    ...
                    <tbody>
                        @foreach($assignments as $a)
                            <tr>
                                ...
                                <td><span class="badge badge-blue">{{ $a->schoolClass?->name }} - {{ $a->section?->name }}</span></td>
                                <td>{{ $a->subject?->name ?? 'General' }}</td>
                                <td>{{ $a->due_date ? date('M d, Y', strtotime($a->due_date)) : 'No due date' }}</td>
                                <td>
                                    @if($a->file_path)
                                        <a href="{{ Storage::disk('public')->url($a->file_path) }}" target="_blank" style="color:#2563eb;font-weight:600;text-decoration:none;"><i class="fas fa-file-download"></i> View File</a>
                                    @else
                                        <span style="color:#9ca3af;">None</span>
                                    @endif
                                </td>
                                <td><span style="font-weight:700;color:#059669;">{{ $a->submissions->count() }} Submissions</span></td>
                                <td>
                                    <form action="{{ route('teacher.assignments.destroy', $a->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this assignment?');" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
            ...
            @else
                <div style="text-align:center;padding:40px;color:#6b7280;">
                    <i class="fas fa-clipboard-list" style="font-size:40px;color:#d1d5db;margin-bottom:12px;"></i>
                    <h4>No Assignments Created Yet</h4>
                    <p style="font-size:13px;margin-top:4px;">Click the button above to create homework assignments for your classes.</p>
                </div>
            @endif
        </div>
    </div>

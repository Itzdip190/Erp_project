...
```
storage/app/snapshots/school_{id}_{code}/
├── 2026-08-30/                  # Date-wise snapshot archive folder (YYYY-MM-DD)
│   ├── school_{id}_{code}_{timestamp}.zip
...
│   │       ├── diary_attachments/...
│   │       └── ...
└── ... (Tiered Retention: 48h Intra-Day Rolling + 15-Day Daily EOD Milestones)
```
...
- **Operation**:
  ...
  - Scans for **Primary Key ID Collisions**: Checks whether any IDs in the snapshot are currently used by _another_ school in production.
...
- **Operation**:
  ...
  6. **Surgical Tenant Purge**: Deletes existing records belonging _only_ to the target `school_id` across all 145 tenant tables in reverse dependency order.
...
| :-------------------------------- | :---------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------- |
| **Full Backup**                   | `php artisan erp:backup-full`                                                 | Creates a complete `.sql.gz` dump + `storage_assets.zip` bundle, verifies SHA-256 integrity, and atomically retains only the latest verified backup. |
...
| **Snapshot Export (Single)**      | `php artisan school:snapshot-export {school_id}`                              | Read-only extraction of an individual school's data (145 tables) and referenced files into a `.zip` bundle. |
| **Snapshot Export (All Schools)** | `php artisan snapshots:export-schools {--clean-older-than=14}`                | Batch multi-school tenant snapshot extraction with fault isolation, summary table, and retention pruning.   |
